...
```mermaid
flowchart TD
    Full["1. 완전 파인튜닝 (Full FT)\n모든 가중치 업데이트 ➔ 16B/param 막대한 VRAM 소모"] --> Partial["2. 부분 파인튜닝 (Partial FT)\n하위 레이어 고정, 상위 레이어만 학습 ➔ 성능 저하 심각 (Figure 7-7)"]
    Partial --> Adapter["3. 직렬 어댑터 (Houlsby, 2019)\n레이어 사이에 작은 병목 MLP 삽입 ➔ ❌ 추론 지연시간 10~30% 증가 (Figure 7-8)"]
    Adapter --> Soft["4. 프롬프트 튜닝 / Prefix Tuning (2021)\n가상 임베딩 벡터 학습 ➔ ❌ 컨텍스트 창 잠식, 소형 모델 취약 (Figure 7-9)"]
    Soft --> LoRA["🏆 5. LoRA (Hu et al., 2021)\n병렬 저차원 행렬 분해 (W + B·A) ➔ 가중치 병합으로 추론 지연시간 0% 달성! (Figure 7-11)"]
```

### ① 초기 PEFT 기법들의 치명적 한계
1. **Houlsby 직렬 어댑터 (Figure 7-8):**  
...
2. **소프트 프롬프트 / 프롬프트 튜닝 (Figure 7-9):**  
   가중치는 고정하고, 입력 토큰 임베딩 앞에 학습 가능한 연속 벡터(가상 토큰 20~100개)를 덧붙여 역전파로 최적화.  
   ❌ **한계:** 사용 가능한 컨텍스트 윈도우 길이를 갉아먹고, 100B 미만의 작은 모델에서는 학습 수렴이 잘 되지 않음.
