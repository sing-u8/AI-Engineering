...
#### 1. 외재적 차원 ($D$) vs 고유 차원 ($d$)의 기하학적 정의
* **외재적 차원 (Extrinsic / Ambient Dimension, $D$):** 모델의 겉보기 파라미터 수 (예: LLaMA-70B라면 $D = 70,000,000,000$차원의 거대한 공간).
* **고유 차원 (Intrinsic Dimension, $d \ll D$):** 데이터를 설명하거나 특정 다운스트림 작업(요약, 분류, SQL 변환 등)을 성공적으로 완수하는 데 필요한 **'최소한의 유효 자유도(Degrees of Freedom)'**.
...
#### 2. 사전 학습(Pre-training)이 고유 차원을 압축하는 원리
* **사전 학습 전 (난수 초기화 / 백지상태):**  
  언어, 문법, 세상의 상식을 전혀 모르는 상태이므로 새로운 작업을 가르치려면 뇌의 모든 축($D$)을 다 건드려야 합니다 ($d_{\text{intrinsic}} \approx D$).
* **사전 학습 진행 과정 (압축기 역할):**  
  수조 개의 텍스트를 학습하면서 트랜스포머는 언어의 문법, 의미적 상관관계, 추론 패턴을 거대한 고차원 공간 속에서 **'매끄러운 저차원 다양체(Low-Dimensional Manifold)'**로 고도로 정렬 및 압축합니다.
* **사전 학습 완료 후 (Foundation Model):**  
  이미 지식과 언어 체계가 99.9% 완성된 20년 차 베테랑 상태이므로, 새로운 도메인 작업에 적응할 때는 **"방향 키를 살짝 꺾어주는 몇 개의 축(LoRA 랭크 $r=2 \sim 8$, 수천~수만 파라미터)"**만 조율하면 즉시 최고 성능을 냅니다.

#### 3. Aghajanyan et al. (2020)의 핵심 실증: 규모의 역설 (Scaling Paradox)
...
> 💡 **핵심 통찰:** 모델이 커질수록 사전 학습을 통해 내부 표현이 더 완벽하게 압축되기 때문에, **파라미터가 큰 모델일수록 LoRA 파인튜닝 시 더 작은 랭크($r=2 \sim 4$)만으로도 완전 파인튜닝과 100% 대등한 성능을 낼 수 있습니다.**

#### 4. 사전 학습에도 LoRA를 처음부터 쓰지 않는 이유 (ReLoRA, GaLore)
...
## 2. LoRA의 수학적 원리와 하이퍼파라미터 구성 (Figure 7-11, Table 7-5, pp. 329 ~ 334)
