...
### ① 다운프로젝션(Down)과 업프로젝션(Up)의 명칭 원리 및 병목 메커니즘
* **행렬 $A$ (다운프로젝션 / Down-projection, $k \to r$):**  
  * 고차원 입력 공간($k = 4,096$)에서 들어온 벡터를 초저차원 병목 공간($r = 8$)으로 **차원을 아래로 축소/압축(Down)**하여 투영하기 때문에 다운프로젝션이라고 부릅니다 (오토인코더의 **인코더/Encoder** 역할).
* **행렬 $B$ (업프로젝션 / Up-projection, $r \to d$):**  
  * 압축된 $r$차원 병목 벡터를 받아 원래 신경망의 고차원 목표 출력 공간($d = 4,096$)으로 **차원을 위로 복원/확장(Up)**하여 투영하기 때문에 업프로젝션이라고 부릅니다 (오토인코더의 **디코더/Decoder** 역할).
* **모래시계형 병목(Bottleneck)의 이점:**  
  * $\text{고차원}(k) \to \text{초저차원}(r) \to \text{고차원}(d)$으로 이어지는 병목 구조를 통과하면서, 모델은 불필요한 노이즈를 버리고 **도메인 적응에 필요한 핵심 지시 특징만 효율적으로 학습**하게 됩니다.

---

### ② 왜 $A \times B$가 아니라 $B \times A$ 순서로 곱해야 하는가? (차원 연산과 코드)
수학 및 코드에서 왜 $A$와 $B$의 곱셈 순서가 **$B \times A$**로 표기되는지에 대한 3가지 핵심 이유입니다:
...
1. **행렬 곱 차원 일치:**  
   * **$B \times A$ 연산:** $(d \times \mathbf{r}) \times (\mathbf{r} \times k) \Longrightarrow \mathbf{d \times k}$ ➔ 원본 $W_0(d \times k)$와 차원이 완벽히 일치하여 덧셈($W_0 + BA$) 가능!
   * **$A \times B$ 연산:** $(r \times \mathbf{k}) \times (\mathbf{d} \times r)$ ➔ $k \neq d$인 경우 **행렬 곱 연산 자체가 불가능(Dimension Mismatch Error)**합니다.
2. **입력 벡터 $x$에 작용하는 연산 순서 (우측 ➔ 좌측):**  
   * 연산 수식 $B \cdot A \cdot x = B \cdot (A \cdot x)$에서, 입력 $x(k \times 1)$는 먼저 $A$와 곱해져 **$r \times 1$ 차원으로 다운프로젝션($A \cdot x$)**됩니다.  
   * 그 다음 결과물이 $B$와 곱해져 **$d \times 1$ 차원으로 업프로젝션($B \cdot (Ax)$)**되어 나가므로, 행렬 표기는 자연스럽게 **$B \cdot A$**가 됩니다.
3. **PyTorch / Hugging Face `peft` 실제 구현:**
   ```python
   # 입력 x가 lora_A를 먼저 통과한 후, 그 출력이 lora_B를 통과
   lora_output = self.lora_B(self.lora_A(x)) * self.scaling
   output = base_layer(x) + lora_output
   ```
