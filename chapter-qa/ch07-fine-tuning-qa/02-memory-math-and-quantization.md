...
* **BF (Bfloat16 / Brain Floating Point, 브레인 부동소수점):**
  ...
  * **결과:** FP32와 완벽히 동일한 범위($10^{38}$)를 다루면서 메모리는 16비트로 절반만 사용하여, **현대 거대 언어 모델(Llama 3, GPT-4, Claude 등) 학습과 파인튜닝의 사실상 표준(Defacto Standard)**이 되었습니다.

```
[ 주요 부동소수점 비트 포맷 구조 (Figure 7-6) ]

• FP32 (32-bit Floating Point) : [ 부호 1비트 ] [ 지수부 8비트 ] [ 가수부 23비트 ] ➔ 초고정밀도 표준 (범위: 10^38)
• FP16 (16-bit Half-Precision)  : [ 부호 1비트 ] [ 지수부 5비트 ] [ 가수부 10비트 ] ➔ 지수부가 작아 오버플로/언더플로 위험 (최댓값 65,504)
