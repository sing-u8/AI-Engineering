...
한국의 AI 스타트업 Upstage가 개발한 **Depthwise Scaling (DWS)**은 모델 병합을 통해 모델의 깊이(Depth)를 확장한 대표적 혁신 사례입니다:

```mermaid
flowchart TD
    subgraph Base32["Llama 2 기반 32 레이어 베이스 모델"]
        L1["Layer 0 ~ 15 (하위 16개 레이어)"]
        L2["Layer 16 ~ 31 (상위 16개 레이어)"]
    end

    subgraph Stack48["Solar-10.7B 48 레이어 확장 구조 (Figure 7-19)"]
        S1["Bottom Block: Layer 0 ~ 23 (24개 레이어)"]
        S2["Top Block: Layer 8 ~ 31 (24개 레이어)"]
    end

    Base32 -->|레이어 중첩 연결| Stack48
    Stack48 --> ContPre["가벼운 계속 사전학습 (DWS Continued Pre-training)"]
    ContPre --> Solar["🏆 Solar-10.7B (당시 13B~30B 오픈소스 모델 벤치마크 압도!)"]
```

1. **레이어 중첩 스태킹:** 32개 레이어를 가진 7B 모델 2개를 복사한 뒤, 중간 16개 레이어를 겹치도록(Layer 0~23 + Layer 8~31) 연결하여 **48개 레이어(10.7B 파라미터)**로 확장합니다.
2. **계속 사전학습 (Continued Pre-training):** 확장 직후 연결 부위의 미세한 불연속성을 정렬하기 위해 가벼운 계속 사전학습을 수행하여, **바닥부터 10B 모델을 학습하는 비용의 1/10도 안 되는 비용으로 당시 30B 모델을 능가하는 초고성능 모델**을 탄생시켰습니다.
