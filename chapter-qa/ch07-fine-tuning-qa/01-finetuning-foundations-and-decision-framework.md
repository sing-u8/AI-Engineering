...
## 📌 핵심 요약 & 전체 맥락
> **"파인튜닝은 '형식과 태도(Form & Behavior)'를 바꾸는 것이고, RAG는 '사실과 지식(Facts & Knowledge)'을 공급하는 것입니다."**  
> 파인튜닝(Fine-Tuning)은 사전 학습된 모델의 가중치를 특정 작업이나 도메인 데이터로 추가 학습시켜 모델의 행동 양식, 응답 톤, 출력 포맷(JSON/SQL)을 영구적으로 교정하는 기법입니다 (Code Llama 파이프라인).  
> 130만 GPU 시간을 쏟아부은 금융 특화 모델 BloombergGPT가 출시 당월 범용 GPT-4-0314에 완패한 실증 사례는 **"도메인 특화 모델보다 프론티어 범용 모델의 스케일링이 더 빠르다"**는 냉혹한 현실을 보여줍니다.  
> 본 섹션에서는 파인튜닝의 본질적 가치, 파인튜닝으로 인한 기존 성능 저하(얼라인먼트 택스 / 파국적 망각), 그리고 **프롬프팅 ➔ RAG ➔ 파인튜닝으로 이어지는 엔지니어링 4분면 의사결정 프레임워크**를 완벽히 정리합니다.
...
* **전이 학습 (Transfer Learning):**  
  수조 개의 인터넷 텍스트로 세상에 대한 일반 지식을 습득한 모델에, 수백~수천 개의 고품질 전문 도메인 예시를 학습시켜 **표본 효율성(Sample Efficiency)**을 극대화하는 기법.
* 💡 **InstructGPT 논문 (OpenAI, 2022)의 통찰:**  
  *"파인튜닝은 모델에 완전히 새로운 능력을 주입하기보다는, 사전 학습 단계에서 모델이 이미 획득했으나 프롬프팅만으로는 꺼내기 힘들었던 잠재 능력을 '잠금 해제(Unlocking)'하는 과정이다."*
...
많은 엔지니어들이 *"우리 분야는 금융/의료/법률이라 특수하므로 반드시 자체 모델을 파인튜닝하거나 처음부터 학습해야 한다"*고 생각합니다.  
하지만 **BloombergGPT의 교훈**은 이 믿음이 틀릴 수 있음을 명백히 보여줍니다:

* **BloombergGPT (2023년 3월):** 500억(50B) 파라미터 금융 전용 모델. A100 GPU **130만 시간**, 연산 비용만 **130만~260만 달러(약 20~35억 원)** 투입.
...
* ⚠️ **얼라인먼트 택스 (Alignment Tax)와 파국적 망각:**  
  모델을 '주문 변경' 작업에 맞춰 파인튜닝하면 주문 변경 성능은 올라가지만, 기존에 잘하던 **'상품 추천'이나 '일반 질의응답' 성능이 급격히 저하(Alignment Tax)**될 수 있습니다.
