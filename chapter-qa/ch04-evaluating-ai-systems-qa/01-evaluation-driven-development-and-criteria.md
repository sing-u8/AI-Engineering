...
### ④ 지시 이행 능력: IFEval & INFOBench (Instruction Following, pp. 173 ~ 175)
자연어의 주관적 평가를 배제하고, **프로그래밍 코드로 100% 엄격하게 정답 여부를 자동 검증(Verifiable)**할 수 있는 벤치마크와 의미론적 제약을 평가하는 벤치마크가 존재합니다.
...
* **INFOBench (내용 및 스타일 제약 검증):**
  * IFEval이 기계적인 포맷만을 검증한다면, INFOBench는 "특정 주제만 다룰 것", "빅토리아 시대 영어 사용", "정중한 어조 유지" 같은 **내용(Content), 언어(Linguistic), 스타일(Style) 제약을 평가**합니다.
  * 프로그래밍으로 자동 검증이 불가능하므로, 제약 조건을 **Yes/No 질문으로 변환하여 AI 판사(GPT-4 등)가 채점**하는 방식을 사용합니다.
...
> ⚠️ **주의:** 건초더미 속 바늘 찾기(NIAH)와 "Lost in the Middle" 현상은 엄밀히 말해 도서의 Chapter 5 (Prompt Engineering, p. 218)에서 주로 다루어지지만, 긴 문맥을 처리하는 모델의 한계를 평가(Evaluation)하는 핵심 기법이므로 이곳에서도 함께 언급합니다.
...
* **"Lost in the Middle" 현상의 치명적 한계 (Liu et al., 2023):**  
