...
### ① 희소(Sparse)와 밀집(Dense)의 기본 구분
* **희소 검색 (Sparse Retrieval):** 사전의 수만 개 단어 중 본문에 등장한 단어 인덱스에만 값이 있고 나머지는 전부 0인 거대한 원-핫(One-hot) 성격의 벡터 (예: `BM25`, `TF-IDF`).
* **밀집 검색 (Dense Retrieval):** 768~1536차원의 압축된 연속 공간에서 거의 모든 원소가 0이 아닌 실수값으로 채워진 벡터 (예: `OpenAI text-embedding-3`, `BERT`).

---

### ② 책에서 '단어 기반 vs 임베딩 기반'으로 재분류한 이유 (SPLADE의 예외)

전통적인 문헌에서는 '희소 vs 밀집'으로 분류하지만, 본 책에서는 **'단어 기반(Term-based) vs 임베딩 기반(Embedding-based)'**이라는 기준을 사용합니다. 그 결정적 이유는 **`SPLADE` (Formal et al., 2021)**라는 특이한 모델 때문입니다:

```
[ SPLADE (Sparse Lexical and Expansion)의 이중적 특성 ]

1. 제작 원리 : BERT 같은 '딥러닝 언어 모델'을 사용해 문맥과 동의어를 학습 (원리는 임베딩 기반)
2. 출력 형태 : 연산 효율성을 위해 L1 정규화로 대부분의 값을 0으로 만든 '희소 벡터' (겉모습은 희소 벡터)
```

* 만약 **겉모습(희소 vs 밀집)**으로만 나누면, SPLADE가 고전적인 `BM25`(단순 단어 카운팅)와 같은 통에 묶여버리는 모순이 발생합니다.
* 하지만 **작동 원리(단어 기반 vs 임베딩 기반)**로 나누면, `BM25`는 **단어 기반**, `SPLADE`와 `Dense Vector`는 **임베딩(신경망) 기반**으로 명확하게 제자리를 찾게 됩니다.

---

### 📊 [한눈에 보는 비교] 알고리즘별 겉모습과 작동 원리

| 알고리즘 | 겉모습 (벡터 형태) | 작동 원리 | 문맥/동의어 이해 | 특징 및 용도 |
| :--- | :---: | :---: | :---: | :--- |
| **BM25 / TF-IDF** | **희소 (Sparse)** | **단어 기반 (Term-based)** | ❌ 불가 | CPU 기반 초고속 키워드 일치 검색 |
| **OpenAI / BGE** | **밀집 (Dense)** | **임베딩 기반 (Embedding-based)** | ✅ 가능 | 의미론적 문맥 및 의도 검색 |
| **SPLADE** *(하이브리드)* | **희소 (Sparse)** | **임베딩 기반 (Embedding-based)** | ✅ 가능 | 신경망 문맥 이해 + 역색인 구조의 빠른 연산 결합 |

> 💡 **❓ [실무 Q&A] "희소가 단어 기반이고 밀집이 임베딩 기반으로 이해하면 되나요?"**  
> **"네, 90% 이상의 일반적인 실무 관점에서는 '희소 = 단어 기반(BM25)', '밀집 = 임베딩 기반(Dense)'으로 이해하셔도 완벽히 맞습니다!"**  
> 다만, SPLADE처럼 *"신경망 임베딩으로 문맥을 학습했지만 표현 형태를 희소 벡터로 다이어트시킨 특수한 예외 모델도 존재한다"*는 점만 기억해 두시면 됩니다.
...
### ① 단어 기반 검색과 역색인 (Term-based: BM25, pp. 258 ~ 261)

#### 1) 단순 키워드 검색의 2대 한계와 TF-IDF 탄생 직관 (p. 258)
질문에 포함된 단어가 들어간 문서를 단순 검색할 때 **2가지 근본적인 문제**가 발생합니다:
1. **너무 많은 문서가 걸리는 문제:**  
   수천 개의 문서가 검색어 단어를 포함할 때 모델의 컨텍스트 창은 제한적입니다. ➔ **단어 빈도 (TF, Term Frequency):** *"단어가 문서 내에 더 많이 등장할수록($f(t, D)$) 해당 문서가 더 관련성이 높다"*는 가설로 순위를 정렬.
2. **질문 속 불필요한 단어가 점수를 왜곡하는 문제:**  
   * 📖 **책 속 실증 쿼리 예시:**  
     `"Easy-to-follow recipes for Vietnamese food to cook at home"` (집에서 만들기 쉬운 베트남 요리 레시피)
   * 이 질문은 9개 단어(`easy-to-follow`, `recipes`, `for`, `vietnamese`, `food`, `to`, `cook`, `at`, `home`)를 포함합니다.
   * 우리가 진짜 찾고 싶은 핵심 단어는 **`vietnamese`**와 **`recipes`**이지만, 거의 모든 영어 문서에 **`for`**, **`to`**, **`at`** 같은 전치사가 압도적으로 많이 등장하여 엉뚱한 문서가 1위로 검색됩니다.
   * ➔ **역문서 빈도 (IDF, Inverse Document Frequency):** **"모든 문서에 흔하게 등장하는 단어일수록 정보 가치가 낮다"**는 원리를 적용하여, 출현 문서 수가 적은 희귀 전문 용어에 높은 가중치를 부여합니다 (예: 전체 10개 문서 중 5개 문서에만 등장하면 $\text{IDF} = 10 / 5 = 2$).

---

#### 2) 역색인 구조 (Inverted Index, Table 6-1)
단어를 키(Key)로 하고, 해당 단어가 등장하는 `(문서 번호, 출현 빈도)`를 값으로 저장하여 밀리초 단위로 초고속 조회하는 자료구조:
...
#### 4) Okapi BM25 알고리즘의 개선점과 파생형 (Robertson et al., 1980s)
* **명칭 유래:** 1980년대 Robertson 등이 개발한 **'Best Matching' 확률적 검색 알고리즘의 25번째 세대 버전**.
* **단어 빈도 포화도 ($k_1$ 파라미터):** 특정 단어가 문서에 100번 반복된다고 해서 점수가 100배로 치솟지 않도록 점수의 상한선(Saturation) 곡선 설정.
* **문서 길이 정규화 ($b$ 파라미터):** 우연히 길이가 길어서 단어가 많이 들어간 장문 문서에 페널티를 부여하고, 짧고 간결하게 핵심 단어를 포함한 문서에 가산점 부여.
* **주요 파생형:** `BM25+`, 제목/본문/요약문 등 문서 필드별로 가중치를 다르게 주는 `BM25F`.
* 💬 **Perplexity CEO (Aravind Srinivas):**  
  > *"BM25나 풀텍스트 검색을 진정으로 뛰어넘는 검색기를 만드는 것은 대단히 어렵다."*

---

#### 5) 토큰화(Tokenization)의 함정과 N-gram 검색의 한계 (pp. 260 ~ 261)
* 📖 **"Hot Dog" 복합어 분해 오류:**  
  단순히 띄어쓰기(공백) 기준으로 질문을 자르면 `"hot dog"`가 `"hot"`(뜨거운)과 `"dog"`(개)로 쪼개져, **원래의 음식 의미를 완전히 유실**합니다. ➔ 빈출 바이그램(Bigram)이나 N-gram(`hot dog`)을 단일 텀으로 묶어주는 전처리가 필요합니다.
* **텍스트 정규화 파이프라인:** 소문자 변환, 구두점 제거, 불용어("the", "and", "is") 필터링을 거치며, **NLTK, spaCy, Stanford CoreNLP** 라이브러리가 주로 사용됩니다.
* **N-gram 중첩 기반 검색의 한계:** 질문과 문서의 N-gram 겹침 점수로 문서를 검색할 수 있지만, **"질문과 문서의 길이가 비슷할 때만 정상 작동"**합니다. 문서가 질문보다 훨씬 길면, 우연히 겹치는 N-gram이 많아져 실제 무관한 긴 문서에 비정상적으로 높은 점수가 매겨집니다.
