...
#### 5) 토큰화(Tokenization)와 N-gram의 개념 및 검색 한계 (pp. 260 ~ 261)

##### ✂️ 토큰화 (Tokenization)란?
* **개념:** 긴 문장이나 문서를 컴퓨터가 처리할 수 있는 가장 작은 의미 단위인 **'토큰(Token, 단어/형태소/글자 조각)'으로 잘게 쪼개는 작업**.
* **비유:** 기다란 기차(문장)를 객차 한 칸 한 칸(토큰)으로 분리하는 작업.
* ⚠️ **단어 검색에서 토큰화의 함정 ("Hot Dog" 분해 오류):**  
  단순히 띄어쓰기(공백) 기준으로만 질문을 자르면, **`"hot dog"`(음식 핫도그)**가 **`"hot"`(뜨거운)**과 **`"dog"`(개)**라는 엉뚱한 2개의 토큰으로 분해되어 검색기가 "뜨거운 개"에 관한 문서를 찾아오는 의미 왜곡이 발생합니다.

---

##### 🔗 N-gram (엔그램)이란?
* **개념:** 문장에서 **"연속된 $N$개의 단어(또는 글자)를 하나의 묶음으로 묶은 것"**입니다.
* $N$의 크기에 따라 다음과 같이 분류됩니다:

| $N$의 크기 | 명칭 | 묶는 방식 | 예시: *"나는 핫도그를 정말 좋아해"* |
| :---: | :---: | :--- | :--- |
| **$N=1$** | **Unigram (유니그램)** | 단어 1개씩 쪼갬 | `["나는"]`, `["핫도그를"]`, `["정말"]`, `["좋아해"]` |
| **$N=2$** | **Bigram (바이그램)** | **연속된 2개씩** 묶음 | `["나는 핫도그를"]`, `["핫도그를 정말"]`, `["정말 좋아해"]` |
| **$N=3$** | **Trigram (트라이그램)** | **연속된 3개씩** 묶음 | `["나는 핫도그를 정말"]`, `["핫도그를 정말 좋아해"]` |

* **N-gram을 사용하는 2대 이유:**
  1. **단어 결합 순서 및 부정어 보존:** 단어 1개씩(Unigram)만 보면 `"not good"`(나쁨)과 `"good"`(좋음)에서 모두 긍정 단어 `"good"`이 잡히지만, Bigram(`["not good"]`)으로 묶으면 부정적인 의미를 그대로 보존할 수 있습니다.
  2. **고유명사 복합어 보호:** `"hot dog"`, `"New York"` 같은 고유명사를 하나의 Bigram 묶음으로 처리하여 의미 분해를 방지합니다.

---

##### ⚠️ N-gram 중첩 기반 검색의 원리와 한계 (긴 문서 왜곡 문제)
* **검색 원리:** 질문의 N-gram 조각들과 문서의 N-gram 조각들이 **얼마나 많이 겹치는지(Overlap)**를 세어서 가장 많이 겹치는 문서를 상위 랭킹으로 추출.
* **치명적 한계 (길이 불균형 왜곡):**  
  * 이 방식은 **"질문과 문서의 길이가 비슷할 때만 정상 작동"**합니다.
  * 만약 문서가 1,000페이지짜리 백과사전처럼 매우 길다면, 질문과 무관한 문서라도 본문 어딘가에서 우연히 겹치는 N-gram 단어쌍이 많아져 **"긴 문서가 부당하게 높은 점수를 받아 1등으로 올라오는 심각한 검색 품질 왜곡"**이 발생합니다.
* **텍스트 정규화 도구:** 단어 기반 검색 전처리를 위해 소문자 변환, 구두점 제거, 불용어("the", "and", "is") 필터링을 수행하며, **NLTK, spaCy, Stanford CoreNLP** 라이브러리가 표준으로 사용됩니다.
