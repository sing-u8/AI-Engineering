...
* **함수 호출 API 제어 모드 (Function Calling Modes):**  
  대부분의 모델 제공자는 도구 사용 여부를 통제할 수 있는 파라미터를 제공합니다:
  * `auto`: 모델이 사용자 질의의 문맥을 파악하여 도구를 사용할지 말지 스스로 결정합니다.
  * `required`: 모델이 반드시 하나 이상의 도구를 호출하도록 강제합니다.
  * `none`: 도구를 사용하지 않고 내부 지식만으로 응답하도록 강제합니다.
...
* **계획 세분성과 번역기 (Planning Granularity & Translator):**  
  계획을 생성할 때 특정 API 함수명(예: `fetch_top_products()`)을 직접 출력하게 하면, API가 변경될 때마다 프롬프트와 파인튜닝 데이터를 전면 수정해야 합니다. 이를 방지하기 위해 에이전트가 상위 수준의 자연어(`1. 인기 상품 검색`)로 계획을 세우게 하고, 이를 실제 실행 가능한 코드로 번역해주는 **번역기 모듈(Program Generator)**을 별도로 두는 것이 시스템의 유지보수성과 환각 방지에 훨씬 유리합니다.
...
* **파운데이션 모델(FM) 플래너 vs 강화학습(RL) 플래너:**  
  전통적인 강화학습(RL) 기반 에이전트는 플래너를 학습시키는 데 막대한 시간과 리소스가 필요합니다. 반면, 파운데이션 모델(FM) 에이전트는 프롬프팅이나 가벼운 파인튜닝만으로도 즉시 강력한 플래너 역할을 수행할 수 있습니다. 장기적으로는 FM 에이전트가 RL 알고리즘을 내부적으로 흡수하는 형태로 융합될 것으로 전망됩니다.
