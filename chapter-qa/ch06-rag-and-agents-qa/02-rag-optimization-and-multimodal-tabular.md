...
#### 3) 계층적 재귀 분할 (Recursive Chunking, p. 268) ⭐
무식하게 고정 글자 수로 잘라서 문장 중간을 훼손하지 않도록, **가장 큰 의미 덩어리부터 점진적으로 쪼개어 내려가는 방식**입니다:
* **1단계 (섹션 / Section 단위 시도):** 먼저 문서의 큰 단원(마크다운 `#`, `##` 대제목/소제목) 단위로 분할을 시도하여, 목표 크기(예: 1,000자) 이내라면 그대로 하나의 청크로 확정합니다.
* **2단계 (문단 / Paragraph 단위 세분화):** 특정 섹션이 너무 길어서 목표 크기를 초과하면, 그 섹션 내부를 **엔터 두 번(`\n\n`) 기준의 문단 단위**로 쪼갭니다.
* **3단계 (문장 / Sentence 단위 최종 분할):** 한 문단 자체가 여전히 너무 길면, 그 안에서 **마침표(`.`) 기준의 개별 문장 단위**로 세분화하여 의미 결합을 최대한 보존합니다.
