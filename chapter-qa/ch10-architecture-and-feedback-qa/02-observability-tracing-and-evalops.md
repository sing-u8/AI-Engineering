...
> 그리고 상태 지속성과 장애 복구를 보장하는 **AI 파이프라인 오케스트레이션 (Airflow, LangChain, LlamaIndex)**의 원리를 완벽하게 정리합니다.
...
다양한 모델, 외부 데이터베이스 검색, 도구 사용 등이 결합된 복잡한 AI 파이프라인에서 데이터가 원활하게 흐르도록 관리하기 위해 **AI 파이프라인 오케스트레이터(AI Pipeline Orchestrator)**가 사용됩니다. 

* **일반 워크플로우 오케스트레이터와의 차이:** 책에서는 AI 파이프라인 오케스트레이터가 Airflow나 Metaflow 같은 일반적인 범용 워크플로우 오케스트레이터와는 다르다고 명확히 구분합니다. 
* **주요 AI 오케스트레이션 도구:** 책에서 언급하는 대표적인 도구로는 **LangChain, LlamaIndex, Flowise, Langflow, Haystack** 등이 있습니다. 검색(Retrieval)과 도구 사용이 흔한 패턴이 되면서 RAG와 에이전트 프레임워크 대부분이 오케스트레이션 역할을 수행합니다.
* **오케스트레이터 도입 시 주의점:** 외부 도구는 시스템을 과도하게 추상화하여 오히려 디버깅을 어렵게 만들 수 있습니다. 따라서 처음에는 오케스트레이터 없이 애플리케이션을 구축해 보고, 파이프라인이 복잡해지는 단계에서 도입을 고려하는 것이 좋습니다.
...
### Q1. 프로덕션 환경에서 분산 트레이싱과 로깅(모든 요청 100% 기록)을 수행하면 리소스 문제가 발생하지 않나요?
모든 데이터를 기록하는 **포괄적 검사(Exhaustive checks)**는 완벽한 가시성을 제공하지만 네트워크 대역폭과 비용을 급증시킬 수 있습니다. 책에서는 이에 대한 대안으로 일부 데이터만 샘플링하여 빠르게 문제를 식별하는 **스팟 검사(Spot checks)**를 제시합니다.
실무적으로는 이 둘을 결합하여 **1) 정상 요청은 일부만 샘플링(Spot check)하여 기록하고, 2) 에러(500 오류), P99 지연시간 초과 요청 등 문제가 발생한 엣지 케이스는 100% 무조건 캡처(Tail-based Sampling)**하는 하이브리드 전략을 통해 모니터링 품질과 비용의 균형을 맞춥니다.
