...
> **1단계(문맥 보강) ➔ 2단계(가드레일 및 PII 마스킹 매핑 테이블) ➔ 3단계(모델 라우터 및 통합 게이트웨이) ➔ 4단계(시스템 캐시 계층) ➔ 5단계(에이전틱 읽기/쓰기 아키텍처)**.
...
| **Figure 10-3** | 입력 시 PII를 가명 토큰으로 치환하고 모델 응답 시 원래 값으로 복원하는 역-사전(Reverse Dictionary) 매핑 구조 | **p. 453** | 2단계: 가드레일과 PII 매핑 테이블 |
| **Figure 10-4** | 입력 가드레일과 출력 가드레일이 모델의 앞뒤를 보호하는 2단계 안전 아키텍처 | **p. 455** | 2단계: 가드레일과 PII 매핑 테이블 |
...
| **Figure 10-8** | 완전 일치와 시맨틱 유사도를 활용하는 4단계 시스템 캐시 아키텍처 | **p. 462** | 4단계: 레이턴시 단축 2중 시스템 캐시 |
...
flowchart TD
    ...
    subgraph Layer4["4단계: 시스템 캐시 계층 (Figure 10-8)"]
        Cache{"캐시 확인\n- Exact Match (Redis)\n- Semantic Cache (Vector DB)\n- Prompt KV Cache"}
    end

    subgraph Layer2_In["2단계: 입력 가드레일 & 보안 (Figure 10-3, 10-4)"]
        PII_Mask["PII 마스킹 및 역-사전(Reverse Dictionary)\n(실제 이름/계좌 ➔ [USER_1] 치환)"]
...
### ② 2단계: 가드레일과 PII 마스킹 매핑 테이블 (Figures 10-3, 10-4) ⭐
...
flowchart LR
    ...
    subgraph Proxy["PII 마스킹 매핑 구조 (Figure 10-3)"]
...
* **PII 마스킹 역-사전(Reverse Dictionary)의 위력 (Figure 10-3):**  
  외부 상용 LLM 벤더(OpenAI, Anthropic 등)로 데이터가 전송될 때 실제 고객의 개인정보가 절대 노출되지 않도록 가명 토큰으로 치환하여 전송하고, 모델의 최종 응답을 사용자에게 보여줄 때만 사내 역-사전(매핑 테이블)을 활용해 원본으로 복원합니다 (GDPR/개인정보보호법 완벽 준수).
...
모든 질의를 단일 모델로 보내는 대신, 목적에 맞는 적절한 솔루션으로 분기합니다:
* **의도 분류기 (Intent Classifier):** 입력된 질문의 목적을 파악하여 적절한 컴포넌트로 라우팅 (예: 비밀번호 재설정 ➔ FAQ 페이지, 환불 문제 ➔ 인간 상담원 전환, 기술적 문제 ➔ 전문 트러블슈팅 모델). 빠르고 저렴한 소형 언어 모델(GPT-2, BERT, Llama 7B 등)을 주로 활용합니다.
* **다음 행동 예측기 (Next-action predictor):** 모델이 외부 검색 API를 호출할지, 코드 인터프리터를 사용할지 등 에이전트의 다음 행동을 결정합니다.
* **비용 최적화 라우팅:** 단순 질의는 초저비용 소형 모델(Llama-3-8B 등)로, 복잡한 추론 질의는 프론티어 모델로 라우팅하여 API 운영 비용을 대폭 절감합니다.
...
### ① 4단계: 시스템 캐시 계층 (Figure 10-8)
시스템 단에서 처리하는 캐시(Figure 10-8)와 모델 API 벤더가 처리하는 캐시로 나눌 수 있습니다:
1. **완전 일치 캐시 (Exact Match Cache):**  
   동일한 프롬프트 문자열이 들어오면 DB(Redis 등)에서 이전 응답을 즉시 반환하여 다단계 작업이나 리소스 집약적인 쿼리의 성능을 크게 향상시킵니다.
2. **시맨틱 캐시 (Semantic Cache):**  
   벡터 검색을 통해 질문의 표현이 다르더라도 의미가 동일하면 기존 캐시를 재사용합니다. 단, 임계값 설정이 까다롭고 '잘못된 캐시 히트(False Positive Hit)' 위험이 있어 주의가 필요합니다.
3. *(참고) 프롬프트 KV 캐시 (Prompt KV Cache):*  
   이는 시스템 캐시(Figure 10-8)에는 나타나지 않지만, 모델 API 제공자(OpenAI, Anthropic 등) 내부에서 공통 문맥의 텐서를 재사용하여 레이턴시와 비용을 줄이는 추론 단의 캐싱 기법입니다.
