...
## 📌 핵심 요약 & 전체 맥락
> **"첫 번째 토큰을 읽을 때는 연산력(Compute)이 부족하고, 이후 토큰을 뱉어낼 때는 메모리 대역폭(Memory Bandwidth)이 모자라다."**  
...
> 본 섹션에서는 하드웨어의 최대 연산력과 메모리 대역폭 간의 관계를 규명하는 **루프라인 모델(Roofline Model: Figure 9-2)**, 실제 서비스의 사용자 경험과 SLA를 결정짓는 **추론 4대 핵심 메트릭(TTFT, ITL/TPOT, Throughput, Goodput: Figure 9-4)**,  
> 그리고 하드웨어의 잠재력을 얼마나 활용하고 있는지 측정하는 **MFU(Model FLOPs Utilization: Table 9-1)**와 **MBU(Model Bandwidth Utilization: Figure 9-5)**, 초고속 SRAM부터 HBM까지 이어지는 **GPU 3단계 메모리 계층(Figure 9-7)**을 완벽하게 정리합니다.
...
하드웨어의 최대 연산 성능은 **피크 연산 성능(TFLOP/s)**과 **메모리 대역폭(Memory Bandwidth, TB/s)**이라는 두 개의 천장에 의해 제한됩니다:
...
* **NVIDIA H100 SXM 변곡점 예시:**
  ...
  * **의미:** 메모리에서 1바이트를 읽어올 때마다 **최소 300회 이상의 수학 연산을 수행해야만 GPU 연산 코어를 100% 활용**할 수 있습니다!
...
flowchart TD
    subgraph Step1["1. 프리필 단계 (Prefill Phase - Prompt Processing)"]
        P_In["입력 프롬프트 1,000 토큰"] --> P_GEMM["행렬-행렬 곱 (GEMM)\n모든 프롬프트 토큰을 병렬로 동시 연산"]
...
1. TTFT (Time to First Token) : 사용자가 프롬프트를 전송한 후 첫 글자가 화면에 뜰 때까지의 지연시간 (프리필 속도).
2. ITL (Inter-Token Latency) / TPOT (Time Per Output Token) : 토큰이 하나씩 스트리밍되어 나오는 간격 (디코딩 속도, 읽기 편안함 결정).
...
* 서비스가 초당 10개의 요청을 완료(Throughput = 10 RPS)했더라도, **SLA 기준(예: TTFT $\le 200\text{ms}$ 및 TPOT $\le 100\text{ms}$)을 만족한 요청이 3건뿐이라면, 실질적인 비즈니스 처리량(Goodput)은 3 RPS**에 불과합니다.

---

## 4. 하드웨어 효율성: MFU와 MBU (Table 9-1, Figure 9-5)

### ① MFU (Model FLOPs Utilization, Table 9-1)
...
### ② MBU (Model Bandwidth Utilization, Figure 9-5)
...
> 💡 **핵심 시사점:**  
> GPU 코어가 아무리 빨라도 데이터를 **HBM(1.5 TB/s)에서 온칩 SRAM(10 TB/s)으로 퍼 올리는 속도**가 느리면 GPU는 대부분의 시간을 멍하니 대기(Idle)하게 됩니다.  
> 따라서 모든 추론 최적화의 제1원칙은 **"데이터가 SRAM에 올라왔을 때 최대한 많은 연산을 끝내고 HBM 접근 횟수를 최소화(IO-Aware Optimization)"**하는 것입니다.
