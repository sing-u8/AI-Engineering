...
> 이 장의 출발점은 추론을 더 빠르고 저렴하게 만드는 방법이다. 최적화는 모델·하드웨어·서비스 수준에서 이루어지며, 속도·비용·품질 사이에는 트레이드오프가 있을 수 있다(p.405).
...
| **02** | [[02-kv-cache-flashattention-and-batching\|02. KV 캐시 수학, FlashAttention 및 연속 배칭]] | 모델 압축·KV 캐시·어텐션 최적화, FlashAttention 커널 융합, PyTorch 사례, 정적/동적/연속 배칭 및 prefill/decode 분리 (pp. 426-443) | `Model Compression`, `KV Cache`, `MQA`, `GQA`, `FlashAttention`, `Continuous Batching`, `Prefill/Decode Disaggregation` |
...
flowchart TD
    ...
    subgraph PartB["2. 모델 수준 최적화 (Model-Level Optimization)"]
        KVCache["KV 캐시 수학 (2×B×S×L×H×M) & GQA 진화"]
        Flash["FlashAttention: 여러 어텐션 연산의 커널 융합"]
        Spec["추측 디코딩 (Draft 모델 검증 / Medusa 다중 헤드 트리)"]
        KVCache --> Flash --> Spec
    end

    subgraph PartC["3. 서빙 인프라 최적화 (Serving Infrastructure)"]
        Batch["연속 배칭 (Continuous/In-flight Batching)"]
        PCache["프롬프트 캐싱 (반복 문맥 재사용: 사용 사례별 비용·지연 감소)"]
...
| **Table 9-2** | NVIDIA H100 SXM의 TF32, BF16, FP16, FP8 TFLOP/s 스펙(희소성 포함) | **p. 422** | 01 |
| **Figure 9-7** | GPU SRAM(20MB, 10TB/s) ➔ GPU HBM(40GB, 1.5TB/s) ➔ CPU DRAM(1TB, 25GB/s)의 예시 메모리 계층 | **p. 424** | 01 |
| **Figure 9-8** | 서비스 제공자의 최적화에 따라 같은 Llama 모델의 벤치마크 결과가 달라질 수 있음을 보이는 비교 그래프 | **p. 426** | 02 |
| **Figure 9-9** | 소형 Draft 모델이 $K$개 토큰을 투기 생성하고 타겟 모델이 1회 병렬 검증하는 추측 디코딩 메커니즘 | **p. 428** | 03 |
| **Figure 9-10** | 입력 문서·코드의 텍스트를 draft로 활용하는 Inference with Reference 예시 | **p. 430** | 03 |
...
| **Figure 9-13** | 여러 어텐션 연산을 하나의 FlashAttention fused kernel로 결합한 비교 | **p. 437** | 02 |
| **Figure 9-14** | Llama-7B에 `torch.compile`, INT8, INT4, speculative decoding을 순차 적용한 PyTorch 처리량 비교 | **p. 439** | 02 |
| **Figure 9-15** | 정적 배칭과 동적 배칭의 대기시간·compute 효율 trade-off | **p. 441** | 02 |
...
| **Table 9-3** | Anthropic 자료의 사용 사례별 캐시 전후 TTFT와 비용 감소 | **p. 444** | 03 |
| **Figure 9-18** | 여러 장치에 행렬을 분할해 병렬 연산하는 텐서 병렬화 (Tensor Parallelism) | **p. 445** | 03 |
...
* **Goodput (유효 처리량):** 서비스 수준 목표(SLO)를 만족한 요청의 처리량. 책의 예시는 TTFT 최대 200ms, TPOT 최대 100ms이며, 10 RPS 중 3 RPS만 SLO를 만족하면 goodput은 3 RPS이다.
...
* **KV Cache (Key-Value 캐시):** 이전 토큰의 Key와 Value 벡터를 저장해 다음 토큰 생성 때 재사용하는 추론 전용 기법. 캐시 크기는 시퀀스 길이와 배치 크기에 따라 선형으로 증가한다.
...
* **FlashAttention (플래시 어텐션):** 어텐션의 여러 연산을 하나의 하드웨어별 fused kernel로 결합해 중간 메모리 접근을 줄이는 커널.
...
* **Prompt Caching (프롬프트 캐싱):** 서로 다른 프롬프트에 반복되는 시스템 프롬프트·문서·대화 구간을 캐시해 재사용하는 기술. 효과는 사용 사례에 따라 다르며, 책의 표에서는 비용이 53~90%, TTFT가 31~79% 감소한다.
