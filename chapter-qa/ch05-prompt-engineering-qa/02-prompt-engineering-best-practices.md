...
예시를 작성할 때 장황한 수식어(`Input:`, `Output:`)를 빼고 기호(`-->`)로만 콤팩트하게 줄이면, 동일한 성능을 내면서도 API(Application Programming Interface) 호출 비용과 지연시간을 크게 아낄 수 있습니다:
...
* 언어 모델은 사람처럼 눈치껏 "아, 여기까지가 질문이고 여기서부터 내 답변을 쓰면 되겠구나"라고 스스로 구분하지 못합니다.
* ⚠️ **구분자 부재 오류 (Table 5-3):**  
  `chicken` 이라는 마지막 단어 뒤에 화살표(`-->`)나 샵(`###`) 같은 **명시적 구분자(Delimiter)**를 주지 않으면, 모델은 정답(`edible`)을 내놓는 대신 `chicken tacos --> edible`처럼 **입력 단어 뒤에 자기 마음대로 단어를 더 이어쓰는 끔찍한 환각 오류**를 범합니다.

#### ⑥ 구조화된 출력(Structured Output)을 강제하는 4단계 엔지니어링 🛠️
단순 텍스트가 아닌, 다른 소프트웨어와 통신하기 위한 완벽한 JSON(JavaScript Object Notation)이나 YAML 형식을 100% 안전하게 뽑아내기 위해, 실무에서는 다음 4가지 방식을 진화 단계별로 사용합니다:
1. **Band-aid (임시방편):** 프롬프트에 *"제발 반드시 유효한 JSON 포맷으로만 출력해줘"*라고 읍소하듯 명시적 지시를 남김. (자주 깨짐)
2. **Post-processing (후처리):** 모델이 뱉은 텍스트에서 Regex(Regular Expression, 정규 표현식) 코드나 파서를 돌려 `{ }` 중괄호 블록만 억지로 뜯어냄.
3. **Test-time compute (재시도 루프):** 파싱 에러가 났을 때 그 에러 메시지를 모델에게 다시 던져주며 "네가 짠 포맷 틀렸으니 다시 고쳐 써라"고 재시도(Retry)시킴.
4. **Tooling (구조화 도구 강제 🏆):** 애초에 모델이 딴소리를 못 하도록, OpenAI의 JSON 모드나 **`Instructor`**(Pydantic 스키마 강제 라이브러) 같은 최신 도구를 사용하여 **지정된 포맷 외의 글자(토큰) 자체를 아예 생성하지 못하도록 원천 차단**함.
...
* 산술 및 다단계 추론 벤치마크(수학 응용문제 등)에서 모델의 정답률이 20%대에서 60~80% 이상으로 수직 상승합니다.
* 💡 **창발적 능력 (Emergent Ability):** 흥미롭게도 CoT는 파라미터가 1,000억 개(100B) 이상인 **초거대 파운데이션 모델에서만 마법처럼 폭발적으로 작동**합니다. 아주 작은 모델은 애초에 논리적으로 생각할 지능이 부족해서 "생각해보자"고 시켜도 엉뚱한 헛소리를 길게 늘어놓을 뿐 큰 효과를 보지 못합니다.
* **LinkedIn의 현장 발견:** CoT를 적용하여 모델이 스스로 검증할 시간을 주면, 복잡한 업무에서 모델이 거짓말을 하는 환각 현상이 크게 줄어듭니다.
...
* **메타 프롬프팅 (Figure 5-7):** 내가 직접 머리를 쥐어짜는 대신, 제일 똑똑한 Claude 3.5 Sonnet 같은 모델에게 *"내가 이런 챗봇을 만들 건데, 너 자신을 위한 최고의 시스템 프롬프트를 직접 작성해줘"*라고 요청하여 초안을 자동 생성받는 방법입니다.
* **Promptbreeder (DeepMind, 2023, Figure 5-8):**  
  진화론의 유전 알고리즘(Genetic Algorithm)을 본떠, 수십 개의 프롬프트 변종(돌연변이)을 만들고 서로 교배시켜 가장 점수가 높은 1등 프롬프트만 살아남게 만드는 딥마인드의 전자동 최적화 시스템입니다.
* 🚀 **DSPy (Demonstrate-Search-Predict Framework):**  
  개발자가 직접 프롬프트 문장을 한 땀 한 땀 깎는 시대는 저물고 있습니다. DSPy는 **프로그램의 뼈대(로직)와 채점 기준만 파이썬 코드로 짜두면, 모델이 훈련 데이터를 돌려보면서 "아, 이런 단어와 이런 예시를 썼을 때 채점 점수가 제일 높구나!"를 스스로 깨닫고 가장 완벽한 퓨샷 예시와 지시문을 자동으로 완성(컴파일, Compile)해 내는 최첨단 패러다임**입니다.
